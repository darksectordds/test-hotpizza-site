        ...
        mounted() {
            if (this.$route.name === null) {
                ...
                if (this.isUrlAutoDetected) {
                    ...
                    // если не изменился родительский idx текущей навигации,
                    // то меняем его, т.е. для начала синхранизируем idx
                    if (this.navigations[this.idx].route !== this.route.name) {

                        // так происходит, когда идет первая загрузка, route был найден,
                        // но он не соответствует выбранному родительскому navigation[idx],
                        // что приведет к невозможности первого нажатия скажем на idx=0
                        const nav_item = _.find(this.navigations, (o) => {
                            return o.route === this.route.name;
                        });

                        this.interfaceIdx(nav_item.key);
                    }

                    // иначе нужно просто открыть и подгрузить
                    // нужный route с правильными параметрами
                    // определенными прямо из url и urlQuery
                    else this.showRoute(this.route);
                }
